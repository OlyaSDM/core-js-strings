...
function findLongestWord(sentence) {
  ...
  if (words.length === 0) return ""; // Handle empty sentences
...
function extractEmails(str) {
  ...
  return str.split(';').map(email => email.trim()).filter(email => email.length > 0);
}
...
function getCardId(value) {
  ...
  if (suitIndex === -1) {
    throw new Error(`Invalid suit in card value: ${value}`);
  }
  ...
  if (rankIndex === -1) {
    throw new Error(`Invalid rank in card value: ${value}`);
  }
